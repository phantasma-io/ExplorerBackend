...
namespace GhostDevs.Tests
{
    ...
    public class JsonTests
    {
        ...
        {
            // Arrange
            var phaChainResponse = @"{""error"" : ""Constraint failed: nft GHOST 88952507113204945896685671555612179007301652577970006258682895062464815798701 does not exist""}";
            var node = JsonDocument.Parse(phaChainResponse);

            // Act
            var result = "";
            if (node.RootElement.TryGetProperty("error", out var property))
        ...
        public void Pha_auctions_chain_response_should_be_parsed_properly()
        {
            ...
            var extensionPeriod = Int32.Parse(node.RootElement.GetProperty("extensionPeriod").GetString());
            var listingFee = Int32.Parse(node.RootElement.GetProperty("listingFee").GetString()) * 100;
        ...
        public void Rpc_request_serialization_works_correctly()
        {
            ...
            var parameters = new object[] { "par1", "par2" };
            ...
            // Assert
            json.ShouldBeEquivalentTo(@"{""jsonrpc"":""2.0"",""method"":""testMethod"",""id"":""1"",""params"":[""par1"",""par2""]}");
        }
    }
}