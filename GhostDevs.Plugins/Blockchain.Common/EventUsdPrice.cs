...
public partial class BlockchainCommonPlugin : Plugin, IDBAccessPlugin
{
    ...
    private int EventUsdPricesFillForArray(MainDbContext databaseContext, Event[] events,
        TokenMethods.TokenPrice[] todaysTokenPrices)
    {
        ...
        for ( var i = 0; i < events.Count(); i++ )
        {
            try
            {
                events[i].PRICE_USD = TokenDailyPricesMethods.Calculate(databaseContext, events[i].ChainId,
                    events[i].DATE_UNIX_SECONDS, events[i].QuoteSymbol.SYMBOL, events[i].PRICE, "USD");

                if ( events[i].PRICE_USD == 0 )
                    events[i].PRICE_USD = ( decimal ) TokenMethods.CalculatePrice(todaysTokenPrices, events[i].PRICE,
                        events[i].QuoteSymbol.SYMBOL);
            }
            catch
            {
                Log.Warning("Event USD price can't be calculated using token price '{Price}'", events[i].PRICE);
            }
    ...
    public void EventUsdPricesFill()
    {
        ...
        using ( var databaseContext = new MainDbContext() )
        {
            ...
            var events = databaseContext.Events
                .Where(x => x.PRICE != null && x.PRICE != "0" && x.PRICE_USD == 0)
                .OrderBy(x => x.TIMESTAMP_UNIX_SECONDS).Take(maxEventUsdPricesProcessedPerSession).ToArray();
