...
namespace GhostDevs.Price
{
    public class CoinGecko : Plugin, IDBAccessPlugin
    {
        ...
        public void LoadPrices(List<TokenMethods.Symbol> cryptoSymbols)
        {
            ...
            var url = "https://api.coingecko.com/api/v3/simple/price?ids=" + string.Join(separator, cryptoSymbols.Where(x => !String.IsNullOrEmpty(x.ApiSymbol)).Select(x => x.ApiSymbol).Distinct().ToList()) + "&vs_currencies=" + string.Join(separator, fiatSymbols);
            ...
            using (var databaseContext = new MainDbContext())
            {
                ...
                {
                    if (!string.IsNullOrEmpty(cryptoSymbol.ApiSymbol) && response.RootElement.TryGetProperty(cryptoSymbol.ApiSymbol.ToLower(), out var priceNode))
                    {
                        foreach (var fiatSymbol in fiatSymbols)
                        {
                            if (priceNode.TryGetProperty(fiatSymbol.ToLower(), out var priceElement))
                            {
                                ...
                                TokenMethods.SetPrice(databaseContext, ChainMethods.GetId(databaseContext, cryptoSymbol.ChainName), cryptoSymbol.NativeSymbol, fiatSymbol, price, false);
                                pricesUpdated++;
                            }
                            else
                            {
                                Log.Warning($"{Name} plugin: Can't find price for '{cryptoSymbol.NativeSymbol}'/'{cryptoSymbol.ApiSymbol}'/'{fiatSymbol}', url: {url}");
                            }
                        }
                    }
                    else
                    {
                        ...
                        if(cryptoSymbol.NativeSymbol.ToUpper() != "GOATI" &&
                            ...
                            Log.Warning($"{Name} plugin: Price for '{cryptoSymbol.NativeSymbol}' crypto is not available.");
                    }
                }
        ...
        public void LoadPricesHistory(List<TokenMethods.Symbol> cryptoSymbols)
        {
            ...
            using (var databaseContext = new MainDbContext())
            {
                ...
                if(lastLoadedDate == 0)
                {
                    // Price history is not loaded yet.
                    lastLoadedDate = UnixSeconds.FromDateTime(Settings.Default.StartDate);
                }
                else
                {
                ...
                {
                    foreach (var cryptoSymbol in cryptoSymbols.Where(x => !String.IsNullOrEmpty(x.ApiSymbol)))
                    {
                        var url = "https://api.coingecko.com/api/v3/coins/" + cryptoSymbol.ApiSymbol + "/history?date=" + lastLoadedDate.ToString("dd-MM-yyyy");

                        var response = Client.APIRequest<JsonDocument>(url, out var stringResponse, (error) => { if (error.ToUpper().Contains("Too Many Requests".ToUpper())) { Log.Information($"{Name} plugin: We reached request limit, saving changes and sleep for 5 seconds."); databaseContext.SaveChanges();  Thread.Sleep(5000); } });
                        if (response == null)
                        {
                            Log.Error($"{Name} plugin: Stuck on '{cryptoSymbol.ApiSymbol}' price update for '{lastLoadedDate.ToString("dd-MM-yyyy")}'.");
                            return;
                        }
                        ...
                        if (response.RootElement.TryGetProperty("market_data", out var marketProperty))
                        {
                            ...
                            {
                                if ((lastLoadedDate >= 1601596800 || cryptoSymbol.NativeSymbol.ToUpper() != "KCAL") && cryptoSymbol.NativeSymbol.ToUpper() != "GOATI")
                                {
                                    Log.Information($"{Name} plugin: Price for '{cryptoSymbol.NativeSymbol}' crypto is not available.");
                                }
                            }
                        }
                    }
            ...
            using (var databaseContext = new MainDbContext())
            {
                ...
                {
                    Dictionary<string, decimal> tokenUsdPrices = new Dictionary<string, decimal>
                    {
                        ["SOUL"] = TokenDailyPricesMethods.Get(databaseContext, ChainMethods.GetId(databaseContext, "main"), firstIncompleteDate, "SOUL", "USD"),
                        ["KCAL"] = TokenDailyPricesMethods.Get(databaseContext, ChainMethods.GetId(databaseContext, "main"), firstIncompleteDate, "KCAL", "USD"),
                        ["GOATI"] = 0.1m
                    };

                    if (tokenUsdPrices["SOUL"] == 0 || tokenUsdPrices["NEO"] == 0 || tokenUsdPrices["ETH"] == 0)
                    {
                        Log.Information($"{Name} plugin: Prices for '{UnixSeconds.LogDate(firstIncompleteDate)}' are not yet available.");
                        break;
                    }
                    ...
                    foreach (var cryptoSymbol in cryptoSymbols.Where(x => !String.IsNullOrEmpty(x.ApiSymbol)))
                    {
                        ...
                        var tokenUsdPrice = TokenDailyPricesMethods.Get(databaseContext, ChainMethods.GetId(databaseContext, cryptoSymbol.ChainName), firstIncompleteDate, cryptoSymbol.NativeSymbol, "USD");
