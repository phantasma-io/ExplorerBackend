...
public partial class Endpoints
{
    ...
    public NftsResult Nfts([APIParameter("Order by [mint_date]", "string")] string order_by = "mint_date",
        ...
        int with_total = 0,
        [APIParameter("Fiat currency to calculate Fiat price", "string")]
        string fiat_currency = "USD",
        [APIParameter("Address of asset creator (multiple values supported, comma-separated)", "string")]
        string creator = "",
        [APIParameter("Address of asset owner (multiple values supported, comma-separated)", "string")]
        string owner = "",
        ...
        string chain = "",
        [APIParameter("Symbol (ex. 'TTRS') (multiple values supported, comma-separated)", "string")]
        string symbol = "",
        [APIParameter("Token ID (multiple values supported, comma-separated)", "string")]
        string token_id = "",
            ...
            try
            {
                ...
                if ( !string.IsNullOrEmpty(symbol) )
                {
                    ...
                    if ( symbol.Contains(",") )
                    {
                        ...
                        var symbols = symbol.ToUpper().Split(',');
                ...
                query.AddSubselect(
                    @"select ""ADDRESS"" as ""SERIES_CREATOR_ADDRESS"" from ""Addresses"" where ""Addresses"".""ID"" = ""Serieses"".""CreatorAddressId""");
                ...
                var resultRoot =
                    pgConnection.ExecQueryDN(query.GetQuery(), query.GetParams());

                if ( with_total == 1 )
                    //DateTime totalStartTime = DateTime.Now;
                    totalResults =
                        ( long ) pgConnection.ExecQueryI(query.GetCountQuery("total_results"), query.GetParams());
                ...
                nftsArray = resultRoot.Select(x =>
                {
                    ...
                    return new Nft
                    {
                        ...
                        creator_address = creatorAddress == null
                            ? null
                            : AddressMethods.Prepend0x(creatorAddress.ADDRESS, x.GetString("NFT_CHAIN_NAME")),
                        creator_onchain_name = creatorAddress == null ? null : creatorAddress.ADDRESS_NAME,
                        owners = databaseContext.NftOwnerships.Where(o => o.NftId == x.GetInt32("ID", 0)).Select(o =>
                            new NftOwnershipResult
                            {
                                address = AddressMethods.Prepend0x(o.Address.ADDRESS,
                                    x.GetString("NFT_CHAIN_NAME", null), true),
                                onchain_name = o.Address.ADDRESS_NAME,
                                offchain_name = o.Address.USER_NAME,
                                amount = o.AMOUNT
                            }).ToArray(),
                        contract = ContractMethods.Prepend0x(x.GetString("CONTRACT_HASH"),
                            x.GetString("NFT_CHAIN_NAME")),
                        nft_metadata = new NftMetadata
                        {
                            name = metadataName,
                            description = x.GetString("DESCRIPTION"),
                            image = x.GetString("IMAGE"),
                            video = x.GetString("VIDEO"),
                            info_url = x.GetString("INFO_URL"),
                            /*rom = x.GetString("ROM", null),
                            ram = x.GetString("RAM", null),*/
                            mint_date = metadataName != null ? x.GetInt64("MINT_DATE_UNIX_SECONDS").ToString() : null,
                            mint_number = metadataName != null ? x.GetString("MINT_NUMBER") : null
                        },
                        series = new Series
                        {
                            id = x.GetString("SERIES_ID"),
                            creator = AddressMethods.Prepend0x(x.GetString("SERIES_CREATOR_ADDRESS"),
                                x.GetString("NFT_CHAIN_NAME")),
                            current_supply = x.GetInt32("SERIES_CURRENT_SUPPLY"),
                            max_supply = x.GetInt32("SERIES_MAX_SUPPLY"),
