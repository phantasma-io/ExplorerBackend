    ...
    public static class ArgValidation
    {
        ...
        public static bool CheckLink(string value)
        {
            ...
            bool result = Uri.TryCreate(value, UriKind.Absolute, out uriResult)
                ...
                || uriResult.Scheme.ToLower() == "ipfs"|| uriResult.Scheme.ToLower() == "ipfs-video");
         ...
         */
        private static string _discordPattern = @"(https?:\/\/)?(www\.)?(discord\.(gg|io|me|li)|discordapp\.com\/invite)\/.+[a-z]";
        private static string _facebookPattern =
            @"(?:https?:\/\/)?(?:www\.)?facebook\.com\/.(?:(?:\w)*#!\/)?(?:pages\/)?(?:[\w\-]*\/)*([\w\-\.]*)";
        private static string _instagramPattern = @"(?:(?:http|https):\/\/)?(?:www.)?(?:instagram.com|instagr.am|instagr.com)\/(\w+)";
        private static string _spotifyPattern =
            @"^(https:\/\/open.spotify.com\/user\/spotify\/playlist\/|spotify:user:spotify:playlist:)([a-zA-Z0-9]+)(.*)$";
        private static string _telegramPattern = @"(https?:\/\/)?(www[.])?(telegram|t)\.me\/([\.a-zA-Z0-9_-]*)\/?$";
        private static string _youtubePattern = @"(https?:\/\/)?(www\.)?youtube\.com\/(channel|user)\/[\w-]+";
        private static string _twitterPattern = @"(?:http:\/\/)?(?:www\.)?twitter\.com\/(?:(?:\w)*#!\/)?(?:pages\/)?(?:[\w\-]*\/)*([\w\-]*)";
         ...
         */
        private static string _usernamePattern = @"^[a-z]{3,}[0-9]*\z";
        /*
         * Title Pattern:
         * Allow all cases.
         ...
         * The max length is checked elsewhere
         */
        private static string _titlePattern = @"^(([a-zA-Z0-9]+(\s[a-zA-Z0-9]+))+\z)";
