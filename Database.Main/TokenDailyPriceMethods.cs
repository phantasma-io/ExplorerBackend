...
namespace Database.Main
{
    public static class TokenDailyPricesMethods
    {
        ...
        public static void Upsert(MainDbContext databaseContext, Int64 dateUnixSeconds, int chainId, string symbol, Dictionary<string, decimal> pricePairs, bool saveChanges = true)
        {
            ...
            var entry = databaseContext.TokenDailyPrices.Where(x => x.Token == token && x.DATE_UNIX_SECONDS == dateUnixSeconds).FirstOrDefault();
            if (entry == null)
            {
        ...
        public static decimal CalculateFromUsd(Dictionary<Int64, TokenDailyPrice> soulDailyTokenPrices, Dictionary<Int64, TokenDailyPrice> outTokenDailyPrices, Int64 dateUnixSeconds, decimal priceInUsd, string outPriceSymbol)
        {
            if (outPriceSymbol.ToUpper() == "USD")
                return priceInUsd; // No calculation is needed.

            var dailyPrices = soulDailyTokenPrices.Where(x => x.Key == dateUnixSeconds);
            ...
            var outTokenDailyPrice = outTokenDailyPrices?.Where(x => x.Key == dateUnixSeconds).Select(x => x.Value).FirstOrDefault();

            var outTokenPriceInUsd = outTokenDailyPrice == null ? 0 : outTokenDailyPrice.PRICE_USD;
            if (outTokenPriceInUsd == 0)
                return 0;
            return priceInUsd / (decimal)outTokenPriceInUsd;
        }
    }
}